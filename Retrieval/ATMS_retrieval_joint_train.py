"""
Train the ATM-S encoder (ShallowFBCSPNet backbone) jointly across all subjects on THINGS-EEG2.

Dataset: THINGS-EEG2
Experiment: Reproducibility

Trains a single shared ATM-S EEG encoder on pooled data from all subjects simultaneously,
without a leave-one-subject-out split. Uses the same combined contrastive loss
(L = lambda * L_CLIP + (1-lambda) * L_MSE) to align EEG with ViT-H-14 CLIP embeddings.

Usage:
    python Retrieval/ATMS_retrieval_joint_train.py
"""
import os
import sys
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='./Data/Preprocessed_data_250Hz', help='Path to preprocessed EEG data root')
