"""
Train and evaluate the ATM-E encoder (EEGNetv4 backbone) on THINGS-EEG2 via contrastive learning.

Dataset: THINGS-EEG2
Experiment: Reproducibility

Trains a subject-specific ATM-E EEG encoder using a combined contrastive loss
(L = lambda * L_CLIP + (1-lambda) * L_MSE) to align EEG representations with
ViT-H-14 CLIP image embeddings. Evaluation uses leave-one-subject-out retrieval.

Usage:
    python Retrieval/ATME_retrieval.py --subject sub-01
"""
import os
import sys
...
import tqdm
from eegdatasets_leaveone_ATME import EEGDataset
...
class ATM_E(nn.Module):    
    def __init__(self, num_channels=63, sequence_length=250, num_subjects=1, num_features=64, num_latents=1024, num_blocks=1, drop_prob=0.25,):
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='./Data/Preprocessed_data_250Hz/ThingsEEG', help='Path to the EEG dataset')
    ...
    parser.add_argument('--name', type=str, default="lr=3e-4_retrieval", help='Experiment name')
    ...
    for sub in subjects:
        ...
        model = globals()[args.encoder_type](63, 250, drop_prob = args.dropout)
        model.to(device)
