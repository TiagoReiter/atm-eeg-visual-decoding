"""
Baseline contrastive EEG-to-image retrieval model without the ATM encoder architecture.

Dataset: THINGS-EEG2
Experiment: Reproducibility

Implements a simpler contrastive retrieval baseline using a patch-embedding EEG
encoder (ShallowNet-inspired conv layers + transformer) trained with the combined
loss (L = lambda * L_CLIP + (1-lambda) * L_MSE) against ViT-H-14 CLIP embeddings.
Serves as a comparison point against the full ATM-S/ATM-E models.

Usage:
    python Retrieval/contrast_retrieval.py --subject sub-01
"""
import os
import sys
