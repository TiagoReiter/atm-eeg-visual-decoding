"""
Train and evaluate the ATM-E encoder (EEGNetv4 backbone) on Alljoined1 via contrastive learning.

Dataset: Alljoined1
Experiment: Generalisability

Trains a subject-specific ATM-E EEG encoder on the Alljoined1 naturalistic scene
...
Usage:
    python Retrieval/ATME_retrieval_alljoined.py --subject sub-01
"""
import os
import sys
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=5000, dropout=0.1):
        ...
        # this yields exactly ceil(d_model/2) entries for sin, floor(d_model/2) for cos
        div_term = torch.exp(
...
class PatchEmbedding(nn.Module):
    def __init__(self, emb_size=40, drop_prob=0.25,):
        super().__init__()
...
class ATM_E(nn.Module):    
    def __init__(self, num_channels=64, sequence_length=334, num_subjects=1, num_features=64, num_latents=1024, num_blocks=1, drop_prob=0.25,):
        super(ATM_E, self).__init__()
        self.attention_model = EEGAttention(num_channels, num_channels, nhead=1)   
        self.subject_wise_linear = nn.ModuleList([nn.Linear(sequence_length, sequence_length) for _ in range(num_subjects)])
        self.enc_eeg = Enc_eeg(emb_size = 40, drop_prob=drop_prob)
...
def main_train_loop(sub, model, train_dataloader, test_dataloader, optimizer, device, 
    ...
    for epoch in range(config.epochs):
        ...
        test_loss, test_accuracy, top5_acc = evaluate_model(model, test_dataloader, device, text_features_test_all, img_features_test_all,k=80)
...
def main():
    ...
    parser.add_argument('--name', type=str, default="lr=3e-4_retrieval_alljoined", help='Experiment name')
    ...
    parser.add_argument('--subjects', nargs ='+', default = ['sub-01', 'sub-02', 'sub-03', 'sub-04', 'sub-05', 'sub-06', 'sub-07', 'sub-08', 'sub-09', 'sub-10'] , required=True, help='List of subject IDs (e.g. sub-08)')
    ...
    for sub in subjects:
        ...
        model = globals()[args.encoder_type](64, 334, drop_prob = args.dropout)
        model.to(device)
        ...
        train_dataset = EEGDataset(
            data_path,
            subjects=[sub] if args.insubject else None,
        ...
        results_dir = f"./outputs/{args.encoder_type}/{sub}/{current_time}_alljoined"
