"""
ATM-S encoder (ShallowFBCSPNet backbone) on THINGS-EEG2 with hyperparameter sweep support.

Dataset: THINGS-EEG2
Experiment: Hyperparameter Robustness

Extends the standard ATM-S training script with configurable hyperparameters
(learning rate, dropout, model capacity, attention heads, encoder layers) exposed
as CLI arguments to support systematic hyperparameter search.
...
from shared.layers.Embed import DataEmbedding
...
class ATMS(nn.Module):    
    def __init__(self, config, num_channels=63, sequence_length=250, num_subjects=2, num_features=64, num_latents=1024, num_blocks=1):
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='./Data/Preprocessed_data_250Hz/ThingsEEG', help='Path to the preprocessed THINGS-EEG2 dataset')
    ...
    parser.add_argument('--name', type=str, default="lr=3e-4_retrieval_org", help='Experiment name')
    ...
    parser.add_argument('--dropout',  type=float, default=0.25, help='Transformer dropout rate')
    parser.add_argument('--d_model',  type=int,   default=250,  help='Transformer embedding dimension')
    ...
    parser.add_argument('--pred_len',type=int, default=250, help='Length of the prediction / reconstruction sequence (unused for retrieval)')   
    parser.add_argument( '--seq_len',type=int,default=250,help='Length of the input EEG sequence (number of time-points)')
    ...
    n_chans   = config.enc_in       # e.g. 63 EEG channels
    n_times   = config.seq_len      # e.g. 250 time-points
    if phase == 'train':
        n_classes = text_features_train_all.shape[0]  # 1654
    else:
        n_classes = text_features_test_all.shape[0]   # 200


    #Changed because code has been rewritten to support scirpts for different learning rate, batchsize, etc
    for sub in subjects:
        if args.encoder_type == "ATMS":
            eeg_model = ATMS(config=config)
        elif args.encoder_type == "EEGNetv4":
            eeg_model = EEGNetv4(
                in_chans=n_chans,
                n_outputs=n_classes,
        ...
        elif args.encoder_type == "EEGConformer":
            eeg_model = EEGConformer(
                n_chans=n_chans,
                n_outputs=n_classes,
                n_times=n_times
            )
        elif args.encoder_type == "EEGITNet":
            eeg_model = EEGITNet(
                n_chans=n_chans,
                n_outputs=n_classes,
                input_window_samples=n_times,
        ...
        results = main_train_loop(sub, current_time, eeg_model, train_loader, test_loader, optimizer, device, 
                                  text_features_train_all, text_features_test_all, img_features_train_all, img_features_test_all, config=config, logger=args.logger)
