"""
Train ATM-S encoder to predict SDXL VAE latent features from EEG signals (low-level reconstruction, Alljoined1).

Dataset: Alljoined1
Experiment: Generalisability

Trains the ATM-S model on naturalistic scene EEG recordings from the Alljoined1 dataset,
regressing per-trial EEG onto SDXL VAE latent representations without trial averaging.
Supports leave-one-subject-out cross-validation and logs metrics to Weights & Biases.
...
import tqdm
from eegdatasets_leaveone_latent_vae_no_average_alljoined import EEGDataset
...
from shared.layers.Embed import DataEmbedding_inverted
...
class Config:
    def __init__(self):
        ...
        self.seq_len = 257                      # Sequence length
        self.pred_len = 257                     # Prediction length
...
class encoder_low_level(nn.Module):
    def __init__(self, num_channels=64 , sequence_length=257, num_subjects=1, num_features=64, num_latents=1024, num_blocks=1):
        ...
        self.rep_dim = 128 #rep_dim = how many features per channel after the linear
        self.num_channels = num_channels
        self.sequence_length = sequence_length
        self.subject_wise_linear = nn.ModuleList([
            nn.Linear(sequence_length, self.rep_dim)
        ...
        in_ch = self.num_channels * self.rep_dim
        # CNN upsampler
        self.upsampler = nn.Sequential(
            nn.ConvTranspose2d(in_ch, 1024, kernel_size=4, stride=2, padding=1),  # (1, 1) -> (2, 2)
            nn.BatchNorm2d(1024),
    ...
    def forward(self, x):
        # x: (B, C, T_full)
        # 1) Truncate to exactly sequence_length time‑points
        x = x[:, :, :self.sequence_length]        # → (B, C, 334)
        # 2) Channel‑wise linear projection to rep_dim
        x = self.subject_wise_linear[0](x)        # → (B, C, 128)
        # 3) Reshape into “images” for the ConvTranspose2d stack
        B, C, R = x.shape                         # B=batch, C=64 channels, R=128 rep_dim
        x = x.view(B, C * R, 1, 1)                # → (B, 64*128, 1, 1)
        # 4) Upsample back out to spatial map
...
def train_model(eegmodel, imgmodel, dataloader, optimizer, device, img_features_all, save_dir, epoch):
    eegmodel.train()
    ...
    for batch_idx, (eeg_data, labels, img, img_features) in enumerate(dataloader):
...
def evaluate_model(eegmodel, imgmodel, dataloader, device, img_features_all, k, save_dir, epoch):
    eegmodel.eval()
    img_features_all = img_features_all.to(device).float()
    total_loss = 0
    mse_loss_fn = nn.MSELoss()
...
def main_train_loop(sub, current_time, eeg_model, img_model, train_dataloader, test_dataloader, optimizer, device, 
    ...
    for epoch in range(config.epochs):
        ...
        train_loss, train_accuracy, features_tensor = train_model(eeg_model, img_model, train_dataloader, optimizer, device, img_features_train_all, save_dir=train_save_dir, epoch=epoch)
        if (epoch +1) % 5 == 0:                    
        ...
        test_loss, test_accuracy, top5_acc = evaluate_model(eeg_model, img_model, test_dataloader, device, img_features_test_all, k=200, save_dir=test_save_dir, epoch=epoch)
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='./Data/Preprocessed_data_250Hz/Alljoined1', help='Path to data')
    parser.add_argument('--output_dir', type=str, default='./Data/Output/Generation/Alljoined1', help='Directory to save output results')
    parser.add_argument('--project', type=str, default='Forschungspraxis-Generation', help='Project name for logging')
    parser.add_argument('--entity', type=str, default="t-reiter-technical-university-of-munich", help='WandB entity name')
    parser.add_argument('--name', type=str, default="lr=3e-4_img_pos_pro_eeg_train_vae_1", help='Experiment name')
    parser.add_argument('--lr', type=float, default=1e-3, help='Learning rate')
    parser.add_argument('--epochs', type=int, default=200, help='Number of training epochs')
    parser.add_argument('--batch_size', type=int, default=10, help='Batch size for training') #Reduced to 10 from 30
    ...
    for sub in subjects:
        ...
        test_loader = DataLoader(test_dataset, batch_size=10, shuffle=True, num_workers=0, drop_last=True) #Reduced to 10 from 20
