"""
PyTorch Dataset for THINGS-EEG2: loads preprocessed EEG and ViT-H-14 CLIP image embeddings.

Dataset: THINGS-EEG2
Experiment: Reproducibility

Implements a leave-one-subject-out Dataset that pairs per-trial EEG signals with
precomputed ViT-H-14 (laion2b) CLIP embeddings of the corresponding THINGS stimulus images.
Used for contrastive training of the ATM-S/ATM-E encoder in Stage 1 of the pipeline.
"""
import torch
...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None, val_size=None):
        print("EEGDataset __init__ called")
        ...
        print("Available subjects:", self.subject_list)
        print("Requested subjects:", self.subjects)
        print("Comparison results:", [sub in self.subject_list for sub in self.subjects])
