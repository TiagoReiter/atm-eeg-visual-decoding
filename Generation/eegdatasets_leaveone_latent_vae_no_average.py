"""
PyTorch Dataset for THINGS-EEG2: loads EEG and SDXL VAE latent features without trial averaging.

Dataset: THINGS-EEG2
Experiment: Reproducibility

Implements a leave-one-subject-out Dataset that pairs individual EEG trials with
precomputed SDXL VAE latent representations of the corresponding THINGS stimulus images.
Supports per-trial (non-averaged) loading for low-level image reconstruction training.
"""
import torch
...
# Load dataset paths from the project-level config
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
config_path = os.path.join(_PROJECT_ROOT, "configs", "thingseeg.json")
...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None):
        ...
        if self.classes is None and self.pictures is None:
            ...
            features_filename = os.path.join(features_folder, 'train_image_latent_512.pt') if self.train else os.path.join(features_folder, 'test_image_latent_512.pt')

            if os.path.exists(features_filename) :
                saved_features = torch.load(features_filename)
                self.text_features = None
                self.img_features = saved_features.get('image_latent', None)
            else:
    ...
    def __getitem__(self, index):
        ...
        print(f"Index: {index}, Text Index: {text_index}, Img Index: {img_index}")
        print(f"Text Length: {len(self.text)}, Img Length: {len(self.img)}, Img Features Length: {len(self.img_features)}")

    # Ensure indices are within valid ranges
        if text_index >= len(self.text):
            text_index = len(self.text) - 1  # Use last valid index

        if img_index >= len(self.img):
            img_index = len(self.img) - 1  # Use last valid index

        if img_index >= len(self.img_features):
            img_index = len(self.img_features) - 1  # Ensure img_features index is valid        
